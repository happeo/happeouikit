...
const table = function(FE) {
  FE = FE && Object.hasOwnProperty.bind(FE)("default") ? FE["default"] : FE;
  ...
  FE.PLUGINS.table = function(editor) {
    ...
    function _hoverCell($table_cell) {
      ...
      $select_size
        ...
        .html(row + " &times; " + col); // Remove hover and fr-active-item class from all cells.
    ...
    function _addAccessibility($popup) {
      ...
      editor.events.on(
        ...
        function(e) {
          ...
          if (
            !editor.popups.isVisible("table.insert") ||
          ...
          if (
            ...
            if ($focused_item.is("span.fr-table-cell")) {
              ...
              if (FE.KEYCODE.ARROW_UP == key_code) {
                ...
                column = Math.min(
                  editor.opts.tableInsertMaxSize - 1,
                  column + 1
                );
    ...
    function _initEditPopup(delayed) {
      ...
      if (editor.opts.tableEditButtons.length > 0) {
        table_buttons = '<div class="fr-buttons">'.concat(
          editor.button.buildList(editor.opts.tableEditButtons),
          "</div>"
        );
        var template = {
          buttons: table_buttons
    ...
    function _initColorsPopup() {
      ...
      if (editor.opts.colorsHEXInput) {
        custom_color =
          ...
          editor.language.translate("HEX Color") +
      ...
      for (var i = 0; i < editor.opts.tableColors.length; i++) {
        ...
        if (editor.opts.tableColors[i] != "REMOVE") {
          ...
          colors_html +=
            ...
            '"><img src="/assets/img/editor-icons/icon-format-color-reset.svg"><span class="fr-sr-only">' +
            editor.language.translate("Clear Formatting") +
            "</span></span>";
        }
      }
    ...
    function _addColorsAccessibility($popup) {
      ...
      editor.events.on(
        ...
        function(e) {
          ...
          if (
            !editor.popups.isVisible("table.colors") ||
            !$focused_item.is("span")
          ) {
            return true;
          }
          ...
          if (FE.KEYCODE.TAB == key_code) {
            ...
            status = !editor.accessibility.focusToolbar(
              $tb,
            ...
            if (FE.KEYCODE.ARROW_UP == key_code) {
              nextIndex =
                (((nextIndex - columns) % dimension) + dimension) % dimension; // Javascript negative modulo bug.
            } else if (FE.KEYCODE.ARROW_DOWN == key_code) {
              nextIndex = (nextIndex + columns) % dimension;
            } else if (FE.KEYCODE.ARROW_LEFT == key_code) {
              nextIndex =
                (((nextIndex - 1) % dimension) + dimension) % dimension; // Javascript negative modulo bug.
            } else if (FE.KEYCODE.ARROW_RIGHT == key_code) {
    ...
    function _refreshColor() {
      ...
      $popup
        ...
        .removeClass("fr-selected-color fr-active-item"); // Find the selected color.

      $popup
        .find('span[data-param1="' + color + '"]')
    ...
    function insert(rows, cols) {
      ...
      var table =
        "<table " +
        (!editor.opts.tableDefaultWidth
          ? ""
          : 'style="width: ' + editor.opts.tableDefaultWidth + ';" ') +
      ...
      for (i = 0; i < rows; i++) {
        ...
        for (j = 0; j < cols; j++) {
          table +=
            ...
            (!editor.opts.tableDefaultWidth
              ? ""
              : ' style="width: ' + cell_width.toFixed(4) + '%;"') +
            ">";
    ...
    function remove() {
      if (selectedCells().length > 0) {
        ...
        editor.selection.setBefore($current_table.get(0)) ||
          editor.selection.setAfter($current_table.get(0));
    ...
    function insertRow(position) {
      ...
      if ($table.length > 0) {
        ...
        if (
          editor.$el.find("th.fr-selected-cell").length > 0 &&
          position == "above"
        ) {
          return;
        } else {
          ...
          for (i = 0; i < map[ref_row].length; i++) {
            ...
            if (
              (position == "below" &&
                ref_row < map.length - 1 &&
                map[ref_row][i] == map[ref_row + 1][i]) ||
              (position == "above" &&
                ref_row > 0 &&
                map[ref_row][i] == map[ref_row - 1][i])
            ) {
              ...
              if (
                i === 0 ||
                (i > 0 && map[ref_row][i] != map[ref_row][i - 1])
              ) {
          ...
          if (
            ...
            $ref_row = $(
              $table.find("tbody").not($table.find("> table tbody"))
            );
                ...
                if (
                  ...
                  if (col === 0) {
                    ...
                    if ($(map[i + 1][col - 1])[0].parentNode) {
                      $(map[i + 1][col - 1])[0].insertAdjacentElement(
                        "afterend",
                        td
                      );
                    }
    ...
    function insertColumn(position) {
      ...
      if ($table.length > 0) {
        ...
        $table.find("th, td").each(function() {
          ...
          $cell.data(
            "old-width",
            ($cell.outerWidth() / $table.outerWidth()) * 100
          );
          ...
          .each(function(index) {
            ...
            while (col_no - 1 < ref_col) {
              ...
              // We want to count this as a column as well.
                ...
                col_no +=
                  parseInt($(map[index][col_no]).attr("colspan"), 10) || 1; // ref_td is one td ahead. Get previous td if we want to insert column after.
            ...
            if (
              (position == "after" && col_no - 1 > ref_col) ||
              (position == "before" &&
                ref_col > 0 &&
                map[index][ref_col] == map[index][ref_col - 1])
            ) {
              // Don't increase twice for rowspan.
              if (
                index === 0 ||
                (index > 0 && map[index][ref_col] != map[index - 1][ref_col])
              ) {
                ...
                $ref_td.css(
                  "width",
                  (
                    ($ref_td.data("old-width") * new_width) / old_width +
                    new_width
                  ).toFixed(4) + "%"
                );
              ...
              if ($row.find("th").length > 0) {
                td =
                  '<th style="width: ' + new_width.toFixed(4) + '%;"><br></th>';
              } else {
                td =
                  '<td style="width: ' + new_width.toFixed(4) + '%;"><br></td>';
        ...
        $table.find("th, td").each(function() {
          ...
          if ($cell.data("old-width")) {
            $cell.css(
              "width",
              (($cell.data("old-width") * new_width) / old_width).toFixed(4) +
                "%"
            );
      ...
      if ($table.length > 0) {
        ...
        if (selection.min_j === 0 && selection.max_j == map[0].length - 1) {
              ...
              if (i === 0 || map[i][j] != map[i - 1][j]) {
                ...
                if ((parseInt($cell.attr("colspan"), 10) || 1) > 1) {
                  ...
                  $cell.css(
                    "width",
                    (
                      (($cell.data("old-width") - _columnWidth(j, map)) * 100) /
                      old_width
                    ).toFixed(4) + "%"
                  );
                  ...
                  if ($row.find("> th, > td").length === 0) {
                    ...
                    if (
                      $row.prev().length === 0 ||
                      $row.next().length === 0 ||
                      $row.prev().find("> th[rowspan], > td[rowspan]").length <
                        $row.prev().find("> th, > td").length
                    ) {
                      $row.remove();
                    }
                  }
          ...
          if (selection.min_j > 0) {
            ...
            editor.selection.setAtEnd(
              map[selection.min_i][selection.min_j - 1]
            );
          } else {
          ...
          $table.find("th, td").each(function() {
            ...
            if ($cell.data("old-width")) {
              $cell.css(
                "width",
                (($cell.data("old-width") * 100) / old_width).toFixed(4) + "%"
              );
              $cell.removeData("old-width");
            }
          });
    ...
    function _updateColspan(min_j, max_j, $table) {
      ...
      if (max_j > min_j) {
        ...
        for (j = min_j; j <= max_j; j++) {
          ...
          first_span = Math.min(
            parseInt(map[0][j].getAttribute("colspan"), 10) || 1,
            max_j - min_j + 1
    ...
    function _updateRowspan(min_i, max_i, $table) {
      ...
      if (max_i > min_i) {
        ...
        for (i = min_i; i <= max_i; i++) {
          ...
          first_span = Math.min(
            parseInt(map[i][0].getAttribute("rowspan"), 10) || 1,
            max_i - min_i + 1
    ...
    function _decreaseCellSpan(
      ...
      for (i = min_i; i <= max_i; i++) {
        for (j = min_j; j <= max_j; j++) {
          ...
          if (
            (i > min_i && map[i][j] == map[i - 1][j]) ||
            (j > min_j && map[i][j] == map[i][j - 1])
          ) {
            continue;
          }
          ...
          if (span > 1) {
            if (span - decrease > 1)
              map[i][j].setAttribute(span_type, span - decrease);
    ...
    function mergeCells() {
      ...
      if (
        ...
        (editor.$el.find("th.fr-selected-cell").length === 0 ||
          editor.$el.find("td.fr-selected-cell").length === 0)
      ) {
        ...
        $first_cell.css(
          "width",
          Math.min(100, (width / $current_table.outerWidth()) * 100).toFixed(
            4
          ) + "%"
    ...
    function splitCellHorizontally() {
      ...
      if (selectedCells().length == 1) {
        ...
        if (current_rowspan > 1) {
          ...
          var col =
            cell_origin.col === 0 ? cell_origin.col : cell_origin.col - 1; // Go back until we find a td on this row. We might have colspans and rowspans.

          while (
            col >= 0 &&
            (map[row][col] == map[row][col - 1] ||
              (row > 0 && map[row][col] == map[row - 1][col]))
          ) {
            col--;
          }
          ...
          if (col == -1) {
            ...
            $(
              $current_table
                .find("tr")
                .not($current_table.find("> table tr"))
                .get(row)
            ).prepend($new_td);
          } else {
          ...
          for (i = 0; i < map[0].length; i++) {
            ...
            if (
              i === 0 ||
              map[cell_origin.row][i] != map[cell_origin.row][i - 1]
            ) {
              ...
              if (!$cell.is($selected_cell)) {
                $cell.attr(
                  "rowspan",
                  (parseInt($cell.attr("rowspan"), 10) || 1) + 1
                );
              }
            }
          }
    ...
    function splitCellVertically() {
      ...
      if (selectedCells().length == 1) {
        ...
        if (current_colspan > 1) {
          ...
          var new_width =
            (_columnsWidth(
              cell_origin.col + new_colspan,
              cell_origin.col + current_colspan - 1,
              map
            ) /
              parent_width) *
            100; // Update colspan for current cell.
          ...
          for (i = 0; i < map.length; i++) {
            ...
            if (
              i === 0 ||
              map[i][cell_origin.col] != map[i - 1][cell_origin.col]
            ) {
    ...
    function _selectionLimits(min_i, max_i, min_j, max_j, map) {
      ...
      for (i = first_i; i <= last_i; i++) {
        // First column.
        if (
          (parseInt($(map[i][first_j]).attr("rowspan"), 10) || 1) > 1 ||
        ...
        if (
          (parseInt($(map[i][last_j]).attr("rowspan"), 10) || 1) > 1 ||
      ...
      for (j = first_j; j <= last_j; j++) {
        // First row.
        if (
          (parseInt($(map[first_i][j]).attr("rowspan"), 10) || 1) > 1 ||
        ...
        if (
          (parseInt($(map[last_i][j]).attr("rowspan"), 10) || 1) > 1 ||
    ...
    function selectCells(firstCell, lastCell) {
      ...
      if ($(firstCell).is(lastCell)) {
        ...
        var limits = _selectionLimits(
          Math.min(firstCellOrigin.row, lastCellOrigin.row),
    ...
    function _mouseDown(e) {
      ...
      if (
        ...
        return true; // Stop table editing if user clicks outside the table.
      ...
      if (!editor.edit.isDisabled() || editor.popups.isVisible("table.edit")) {
        ...
        if (
          e.which == 1 &&
          !(e.which == 1 && editor.helpers.isMac() && e.ctrlKey)
        ) {
          ...
          if (cell) {
            ...
            if (
              e.shiftKey &&
              editor.$el.find(tag_name + ".fr-selected-cell").length > 0
            ) {
              // Cells must be in the same table.
              if (
                $(
                  editor.$el
                    .find(tag_name + ".fr-selected-cell")
                    .closest("table")
                ).is($(cell).closest("table"))
              ) {
              ...
              if (
                (editor.keys.ctrlKey(e) || e.shiftKey) &&
                (selectedCells().length > 1 ||
                  ($(cell).find(editor.selection.element()).length === 0 &&
                    !$(cell).is(editor.selection.element())))
              ) {
                _clearSelection();
    ...
    function _mouseUp(e) {
      ...
      if (
        !mouseDownCellFlag &&
        !editor.$tb.is(e.target) &&
        !editor.$tb.is($(e.target).closest(".fr-toolbar"))
      ) {
        if (selectedCells().length > 0) {
          editor.toolbar.enable();
        }
      ...
      if (
        ...
        if (resizingFlag) {
          ...
          var left =
            parseFloat($resizer.css("left")) +
            editor.opts.tableResizerOffset +
            editor.$wp.offset().left;
    ...
    function _mouseEnter(e) {
      if (
        mouseDownCellFlag === true &&
        editor.opts.tableEditButtons.length > 0
      ) {
    ...
    function _moveInNestedTable(cell, direction) {
      ...
      if (table && table.tagName == "TABLE") {
        ...
        if (direction == "up")
          _moveUp(_cellOrigin(cell, new_map), table, new_map);
        else if (direction == "down")
          _moveDown(_cellOrigin(cell, new_map), table, new_map);
      }
    }
    /*
    ...
    function _moveDown(origin, table, map) {
      ...
      var row =
        parseInt(map[origin.row][origin.col].getAttribute("rowspan"), 10) || 1; // Not the last line.
            ...
            if (
              ...
              if (
                ...
                if (
                  ...
                  ((FE.KEYCODE.ARROW_UP == key_code && node.previousSibling) ||
                    (FE.KEYCODE.ARROW_DOWN == key_code && node.nextSibling))
                ) {
                  return;
                }
    ...
    function _initResizer() {
      ...
      if (!editor.shared.$table_resizer)
        editor.shared.$table_resizer = $(document.createElement("div"))
    ...
    function _hideInsertHelper() {
      if ($insert_helper)
        $insert_helper.removeClass("fr-visible").css("left", "-9999px");
    }
    /*
    ...
    function _placeResizer(e, tag_under) {
      ...
      if (
        tag_under &&
        (tag_under.tagName == "TD" || tag_under.tagName == "TH")
      ) {
        ...
        if (
          Math.abs(e.pageX - tag_left) <= editor.opts.tableResizerOffset ||
          ...
          if (editor.opts.direction != "rtl") {
            ...
            if (e.pageX - tag_left <= editor.opts.tableResizerOffset) {
              ...
              if (tag_origin.col > 0) {
                ...
                max_left =
                  tag_left -
                  _columnWidth(tag_origin.col - 1, map) +
                  editor.opts.tableResizingLimit; // Right limit.

                max_right =
                  tag_left +
                  _columnWidth(tag_origin.col, map) -
                  editor.opts.tableResizingLimit; // Columns to resize.
                ...
                max_right =
                  ...
                  1 +
                  $table.width() -
                  map[0].length * editor.opts.tableResizingLimit;
              }
              ...
              if (
                tag_end.col < map[tag_end.row].length &&
                map[tag_end.row][tag_end.col + 1]
              ) {
                // Left limit.
                max_left =
                  tag_right -
                  _columnWidth(tag_end.col, map) +
                  editor.opts.tableResizingLimit; // Right limit.

                max_right =
                  tag_right +
                  _columnWidth(tag_end.col + 1, map) -
                  editor.opts.tableResizingLimit; // Columns to resize.
                ...
                max_left =
                  ...
                  map[0].length * editor.opts.tableResizingLimit;
                max_right =
                  $table_parent.offset().left -
                  1 +
                  $table_parent.width() +
                  parseFloat($table_parent.css("padding-left"));
              }
            }
          } // RTL
          else {
          ...
          var left =
            resizer_left -
            editor.win.pageXOffset -
            editor.opts.tableResizerOffset -
            editor.$wp.offset().left;
          var top =
            resizer_top - editor.$wp.offset().top + editor.$wp.scrollTop();

          if (editor.opts.iframe) {
            var iframePaddingTop = editor.helpers.getPX(
              editor.$wp.find(".fr-iframe").css("padding-top")
            );
            var iframePaddingLeft = editor.helpers.getPX(
              editor.$wp.find(".fr-iframe").css("padding-left")
            );
        ...
        $resizer &&
        $tag_under.get(0) != $resizer.get(0) &&
        $tag_under.parent().get(0) != $resizer.get(0)
      ) {
        if (editor.core.sameInstance($resizer)) _hideResizer();
      }
    }
    ...
    function _showInsertColHelper(e, table) {
      ...
      if (editor.opts.iframe) {
        ...
        left +=
          editor.$iframe.offset().left -
          editor.helpers.scrollLeft() +
          iframePaddingLeft;
        top +=
          editor.$iframe.offset().top -
      ...
      $row.find("th, td").each(function() {
        ...
        if (
          $td.offset().left <= mouseX &&
          mouseX < $td.offset().left + $td.outerWidth() / 2
        ) {
          btn_width = parseInt($insert_helper.find("a").css("width"), 10);
          $insert_helper.css(
            "top",
            top + $td.offset().top - editor.$box.offset().top - btn_width - 5
          );
          $insert_helper.css(
            "left",
            left + $td.offset().left - editor.$box.offset().left - btn_width / 2
          );
          ...
          $td.offset().left + $td.outerWidth() / 2 <= mouseX &&
          mouseX < $td.offset().left + $td.outerWidth()
        ) {
          btn_width = parseInt($insert_helper.find("a").css("width"), 10);
          $insert_helper.css(
            "top",
    ...
    function _showInsertRowHelper(e, table) {
      ...
      $table.find("tr").each(function() {
        ...
        if (
          $tr.offset().top <= mouseY &&
          mouseY < $tr.offset().top + $tr.outerHeight() / 2
        ) {
          btn_width = parseInt($insert_helper.find("a").css("width"), 10);
          $insert_helper.css(
            "top",
            top + $tr.offset().top - editor.$box.offset().top - btn_width / 2
          );
          $insert_helper.css(
            "left",
            left + $tr.offset().left - editor.$box.offset().left - btn_width - 5
          );
          ...
          $tr.offset().top + $tr.outerHeight() / 2 <= mouseY &&
          mouseY < $tr.offset().top + $tr.outerHeight()
        ) {
          btn_width = parseInt($insert_helper.find("a").css("width"), 10);
          $insert_helper.css(
            "top",
    ...
    function _insertHelper(e, tag_under) {
      ...
      if (selectedCells().length === 0) {
        ...
        if (
          tag_under &&
          (tag_under.tagName == "HTML" ||
            tag_under.tagName == "BODY" ||
            editor.node.isElement(tag_under))
        ) {
          ...
          for (i = 1; i <= editor.opts.tableInsertHelperOffset; i++) {
            // Look for tag below.
            tag_below = editor.doc.elementFromPoint(
              e.pageX - editor.win.pageXOffset,
              e.pageY - editor.win.pageYOffset + i
            ); // We're on tooltip.
            ...
            if (
              tag_below &&
              (tag_below.tagName == "TH" ||
                tag_below.tagName == "TD" ||
                tag_below.tagName == "TABLE") &&
              ($(tag_below).parents(".fr-wrapper").length || editor.opts.iframe)
            ) {
              // Show the insert column helper button.
            ...
            tag_right = editor.doc.elementFromPoint(
              e.pageX - editor.win.pageXOffset + i,
              e.pageY - editor.win.pageYOffset
            ); // We're on tooltip.
            ...
            if (
              tag_right &&
              (tag_right.tagName == "TH" ||
                tag_right.tagName == "TD" ||
                tag_right.tagName == "TABLE") &&
              ($(tag_right).parents(".fr-wrapper").length || editor.opts.iframe)
            ) {
              // Show the insert row helper button.
    ...
    function _tagUnder(e) {
      ...
      var tag_under = editor.doc.elementFromPoint(
        e.pageX - editor.win.pageXOffset,
        e.pageY - editor.win.pageYOffset
      ); // Place table resizer if necessary.

      if (
        editor.opts.tableResizer &&
        (!editor.popups.areVisible() ||
          (editor.popups.areVisible() && editor.popups.isVisible("table.edit")))
      ) {
        _placeResizer(e, tag_under);
      } // Show the insert column / row helper button.

      if (
        editor.opts.tableInsertHelper &&
        !editor.popups.areVisible() &&
        !(editor.$tb.hasClass("fr-inline") && editor.$tb.isVisible())
      ) {
        ...
        if (first != null && second != null) {
          ...
          for (i = 0; i < map.length; i++) {
            ...
            if (map[i][first] != map[i][second]) {
              ...
              var first_cell_percentage = (
                (first_percentages[i] *
                  (first_widths[i] + release_position - initial_positon)) /
                first_widths[i]
              ).toFixed(4);
              ...
              $second_cell.css(
                ...
                (
                  first_percentages[i] +
                  second_percentages[i] -
                  first_cell_percentage
                ).toFixed(4) + "%"
              );
            }
          }
          ...
          var left_margin =
            ((parseInt($table.css("margin-left"), 10) || 0) /
              $table_parent.width()) *
            100;
          var right_margin =
            ((parseInt($table.css("margin-right"), 10) || 0) /
              $table_parent.width()) *
            100;
          var width; // Right border RTL or LTR.

          if (
            (editor.opts.direction == "rtl" && second === 0) ||
            (editor.opts.direction != "rtl" && second !== 0)
          ) {
            width =
            ...
            $table.css(
              "margin-right",
              "calc(100% - " +
                Math.round(width).toFixed(4) +
                "% - " +
                Math.round(left_margin).toFixed(4) +
                "%)"
            );
            ...
            $table.css(
              ...
              "calc(100% - " +
                Math.round(width).toFixed(4) +
                "% - " +
                Math.round(right_margin).toFixed(4) +
                "%)"
            );
          } // Update table width.
    ...
    function _mouseMove(e) {
      ...
      if (
        ...
        if (
          ...
          editor.popups.isVisible("table.edit")
        ) {
        ...
        if (pos >= left_limit && pos <= right_limit) {
          $resizer.css(
            "left",
            pos - editor.opts.tableResizerOffset - editor.$wp.offset().left
          ); // Cursor has exceeded the left limit. Don't update if it already has the correct value.
        } else if (
          pos < left_limit &&
          parseFloat($resizer.css("left"), 10) >
            left_limit - editor.opts.tableResizerOffset
        ) {
          $resizer.css(
            "left",
            left_limit -
          ...
          pos > right_limit &&
          parseFloat($resizer.css("left"), 10) <
            right_limit - editor.opts.tableResizerOffset
        ) {
          $resizer.css(
            "left",
            right_limit -
    ...
    function _useTab(e) {
      ...
      if (key_code == FE.KEYCODE.TAB) {
        ...
        if ($cell) {
          ...
          if (
            $(editor.selection.element()).parentsUntil(editor.$el, "ol, ul")
              .length > 0 &&
            ($(editor.selection.element())
              .parents("li")
              .prev().length > 0 ||
              ($(editor.selection.element()).is("li") &&
                $(editor.selection.element()).prev().length > 0))
          ) {
            return true;
          }
          ...
          if (e.shiftKey) {
            ...
            if ($cell.prev().length > 0) {
              ...
              $cell.closest("tr").length > 0 &&
              $cell.closest("tr").prev().length > 0
            ) {
              _addMarkersInCell(
                $cell
                  .closest("tr")
              ...
              $cell.closest("tbody").length > 0 &&
              $cell.closest("table").find("thead tr").length > 0
            ) {
              _addMarkersInCell(
                $cell
                  .closest("table")
            ...
            if ($cell.next().length > 0) {
              ...
              $cell.closest("thead").length > 0 &&
              $cell.closest("table").find("tbody tr").length > 0
            ) {
              _addMarkersInCell(
                $cell
                  .closest("table")
    ...
    function _initInsertHelper() {
      ...
      if (!editor.shared.$ti_helper) {
        editor.shared.$ti_helper = $(document.createElement("div"))
      ...
      if (selection.length > 0) {
        ...
        if (FE.KEYCODE.ARROW_RIGHT == key_code) {
          if (handlerOrigin.col < map[0].length - 1) {
            selectCells(
              fixedCell,
              map[handlerOrigin.row][handlerOrigin.col + 1]
            );
            return false;
          }
        } // Select row below.
          ...
          if (handlerOrigin.row < map.length - 1) {
            selectCells(
              fixedCell,
              map[handlerOrigin.row + 1][handlerOrigin.col]
            );
            return false;
          }
      ...
      if (!editor.helpers.isMobile()) {
          ...
          function(e) {
            ...
            if (selected_cells.length > 0) {
              ...
              if (
                ...
                (e.which == FE.KEYCODE.BACKSPACE ||
                  e.which == FE.KEYCODE.DELETE)
              ) {
                editor.undo.saveStep();
              ...
              if (
                selected_cells.length > 1 &&
                e.which != FE.KEYCODE.F10 &&
                !editor.keys.isBrowserAction(e)
              ) {
                e.preventDefault();
                selected_cells = [];
                return false;
        ...
        editor.events.on("html.afterGet", function() {
          for (var i = 0; i < c_selected_cells.length; i++) {
            c_selected_cells[i].className =
              (c_selected_cells[i].className
                ? c_selected_cells[i].className.trim() + " "
                : "") + "fr-selected-cell";
          }
  ...
  FE.RegisterCommand("tableHeader", {
    ...
    callback: function callback() {
      var $btn = this.popups
        .get("table.edit")
        .find('.fr-command[data-cmd="tableHeader"]'); // If button is active the table has a header,
    ...
    refresh: function refresh($btn) {
      ...
      if (selected_cells.length) {
        $btn
          ...
          .replaceWith(
            this.icon.create(
              "align-" + this.helpers.getAlignment($(selected_cells[0]))
            )
          );
      }
    },
