...
const codeView = function(FE) {
  "use strict";
  FE = FE && Object.hasOwnProperty.bind(FE)("default") ? FE["default"] : FE;

  Object.assign(FE.DEFAULTS, {
    codeMirror:
      (typeof window !== "undefined" && window.CodeMirror) || codeMirror,
    codeMirrorOptions: {
      lineNumbers: true,
      tabMode: "indent",
      indentWithTabs: true,
  ...
  FE.PLUGINS.codeView = function(editor) {
    ...
    function refresh() {
      if (isActive()) {
        if (code_mirror) {
          code_mirror.setSize(
            null,
            editor.opts.height ? editor.opts.height : "auto"
          );
        }
        ...
        if (editor.opts.heightMin || editor.opts.height) {
          editor.$box
            ...
            .css("min-height", editor.opts.heightMin || editor.opts.height);
        } else {
          editor.$box
            .find(".CodeMirror-scroll, .CodeMirror-gutters")
            .css("min-height", "");
    ...
    function _showHTML($btn) {
      if (!$html_area) {
        ...
        if (!code_mirror && editor.opts.codeMirror) {
          code_mirror = editor.opts.codeMirror.fromTextArea(
            $html_area.get(0),
            editor.opts.codeMirrorOptions
          );
      ...
      if (editor.codeBeautifier) {
        html = editor.codeBeautifier.run(
          html,
          editor.opts.codeBeautifierOptions
        );
      }
      ...
      if (code_mirror) {
        ...
        var s_line =
          html.substring(0, s_index).length -
          html.substring(0, s_index).replace(/\n/g, "").length;
        var e_line =
          html.substring(0, e_index).length -
          html.substring(0, e_index).replace(/\n/g, "").length;
        s_index =
          html.substring(0, s_index).length -
          html.substring(0, html.substring(0, s_index).lastIndexOf("\n") + 1)
            .length;
        e_index =
          html.substring(0, e_index).length -
          html.substring(0, html.substring(0, e_index).lastIndexOf("\n") + 1)
            .length;
        code_mirror.setSize(
          null,
          editor.opts.height ? editor.opts.height : "auto"
        ...
        if (editor.opts.heightMin)
          editor.$box
            ...
            .css("min-height", editor.opts.heightMin);
        ...
        if (editor.opts.heightMax) {
          $html_area.css(
            "max-height",
            editor.opts.height || editor.opts.heightMax
          );
        }

        $html_area
          .val(html.replace(/FROALA-SM/g, "").replace(/FROALA-EM/g, ""))
          .trigger("change");
      ...
      editor.$tb
        ...
        .filter(function() {
          return (
            editor.opts.codeViewKeepActiveButtons.indexOf($(this).data("cmd")) <
            0
          );
        })
    ...
    function _refreshToolbar() {
      ...
      if (!isActive()) {
        ...
        editor.$tb
          ...
          .filter(function() {
            return (
              editor.opts.codeViewKeepActiveButtons.indexOf(
                $(this).data("cmd")
              ) < 0
            );
          })
    ...
    function _initArea() {
      ...
      if (!editor.$box.hasClass("fr-basic")) {
        $back_button = $(
          '<a data-cmd="html" title="Code View" class="fr-command fr-btn html-switch fr-desktop" role="button" tabIndex="-1">' +
            editor.language.translate("Save changes") +
            "</a>"
        );
