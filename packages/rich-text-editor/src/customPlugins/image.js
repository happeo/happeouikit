...
const image = function(FE) {
  ...
  FE.PLUGINS.image = function(editor) {
    ...
    function _convertClassesToStyle($img) {
      ...
      var d = $img.hasClass("fr-dib")
        ? "block"
        : $img.hasClass("fr-dii")
        ? "inline"
        : null;
      var a = $img.hasClass("fr-fil")
        ? "left"
        : $img.hasClass("fr-fir")
        ? "right"
        : getAlign($img);
    ...
    function _refreshImageList() {
      var images =
        editor.el.tagName == "IMG"
          ? [editor.el]
          : editor.el.querySelectorAll("img");
      ...
      for (var i = 0; i < images.length; i++) {
        ...
        if (!editor.opts.htmlUntouched && editor.opts.useClasses) {
          if (
            editor.opts.imageDefaultAlign ||
            editor.opts.imageDefaultDisplay
          ) {
    ...
    function _syncImages(loaded) {
      ...
      var c_images = Array.prototype.slice.call(
        editor.el.querySelectorAll("img")
      ); // Current images src.
      ...
      for (i = 0; i < c_images.length; i++) {
        ...
        if (c_images[i].getAttribute("class") === "")
          c_images[i].removeAttribute("class");
        if (c_images[i].getAttribute("style") === "")
          c_images[i].removeAttribute("style");

        if (
          c_images[i].parentNode &&
          c_images[i].parentNode.parentNode &&
          editor.node.hasClass(
            c_images[i].parentNode.parentNode,
            "fr-img-caption"
          )
        ) {
    ...
    function _repositionResizer() {
      ...
      var wrap_correction_top =
        $container.scrollTop() -
        ($container.css("position") != "static" ? $container.offset().top : 0);
      var wrap_correction_left =
        $container.scrollLeft() -
        ($container.css("position") != "static" ? $container.offset().left : 0);
      wrap_correction_left -= editor.helpers.getPX(
        $container.css("border-left-width")
      );
      wrap_correction_top -= editor.helpers.getPX(
        $container.css("border-top-width")
      );
      ...
      if (editor.opts.iframe) {
        iframePaddingTop = editor.helpers.getPX(
          editor.$wp.find(".fr-iframe").css("padding-top")
        );
        iframePaddingLeft = editor.helpers.getPX(
      ...
      $image_resizer
        .css(
          "top",
          (editor.opts.iframe
            ? $el.offset().top + iframePaddingTop
            : $el.offset().top + wrap_correction_top) - 1
        )
        .css(
          "left",
          (editor.opts.iframe
            ? $el.offset().left + iframePaddingLeft
            : $el.offset().left + wrap_correction_left) - 1
        )
    ...
    function _handlerMousedown(e) {
      ...
      // #Universe
      // Prevent resize if has no-resize class
      var parent = e.currentTarget.offsetParent;
      if ($current_image.hasClass("no-resize")) {
        // Prevent resizing if has no-resize
        // add class to indicate to users
        if (
          !!parent.className &&
          typeof parent.className === "string" &&
          parent.className.indexOf("fr-image-resizer") !== -1
        ) {
          parent.className += " fr-resize-denied";
          return false;
        }
      }
      ...
      if (
        !!parent.className &&
        typeof parent.className === "string" &&
        parent.className.indexOf("fr-resize-denied") !== -1
      ) {
        // Remove denied class
        parent.className = parent.className.replace(" fr-resize-denied", "");
      }
      ...
      if (e.type == "mousedown") {
        ...
        try {
          editor_inside_iframe =
            win.location != win.parent.location && !(win.$ && win.$.FE);
        } catch (ex) {}

        if (editor_inside_iframe && win.frameElement) {
          start_x +=
            editor.helpers.getPX($(win.frameElement).offset().left) +
            win.frameElement.clientLeft;
        }
      }
      ...
      if (editor.opts.imageResizeWithPercent) {
        var p_node =
          $current_image
            .parentsUntil(editor.$el, editor.html.blockTagsQuery())
            .get(0) || editor.el;
        ...
        if (editor.opts.imageResizeWithPercent) {
          var p_node =
            $current_image
              .parentsUntil(editor.$el, editor.html.blockTagsQuery())
              .get(0) || editor.el;
          width = (((width + diff_x) / $(p_node).outerWidth()) * 100).toFixed(
            2
          );
          ...
          if (hasCaption()) {
            real_image_size = (
              (editor.helpers.getPX(
                $current_image.parents(".fr-img-caption").css("width")
              ) /
                $(p_node).outerWidth()) *
              100
            ).toFixed(2);
            ...
            real_image_size = (
              (editor.helpers.getPX($current_image.css("width")) /
                $(p_node).outerWidth()) *
              100
            ).toFixed(2);
          ...
          if (width + diff_x >= editor.opts.imageMinWidth) {
            ...
            if (hasCaption()) {
              real_image_size = editor.helpers.getPX(
                $current_image.parents(".fr-img-caption").css("width")
              );
            } else {
              real_image_size = editor.helpers.getPX(
                $current_image.css("width")
              );
            }
          } // If the width is not contained within editor use the real image size.
          ...
          if (
            ($current_image.attr("style") || "").match(
              /(^height:)|(; *height:)/
            ) ||
            ...
            $current_image.css(
              "height",
              ($handler.data("start-height") * $current_image.width()) /
                $handler.data("start-width")
            );
    ...
    function _throwError(code, response, $img) {
      ...
      _showErrorMessage(
        editor.language.translate("Something went wrong. Please try again.")
    ...
    function _setProgressMessage(message, progress) {
      ...
      if ($popup) {
        ...
        $layer
          .find("h3")
          .text(message + (progress ? " " + progress + "%" : ""));
    ...
    function insertByURL() {
      ...
      if ($input.val().length > 0) {
        ...
        if (
          editor.opts.imageUploadRemoteUrls &&
          editor.opts.imageCORSProxy &&
          editor.opts.imageUpload
        ) {
    ...
    function insert(link, sanitize, data, $existing_img, response) {
      ...
      image.onload = function() {
        ...
        if ($existing_img) {
          if (!editor.undo.canDo() && !$existing_img.hasClass("fr-uploading"))
            editor.undo.saveStep();
          ...
          if (editor.$wp) {
            ...
            $img = $existing_img
              ...
              .removeAttr("data-fr-image-pasted"); // Remove load event.
          ...
          editor.events.disableBlur();
          editor.$el.blur();
          editor.events.trigger(old_src ? "image.replaced" : "image.inserted", [
            $img,
            response
          ]);
        } else {
    ...
    function _parseResponse(response) {
      try {
        if (
          editor.events.trigger("image.uploaded", [response], true) === false
        ) {
          editor.edit.on();
          return false;
        }
        ...
        // Universe image upload returns absolute url for images
        // This changes the images uploaded to be relative
        // TODO: This should be done correctly in image service
        var universeSettings = window.universeSettings;
        resp.link = resp.url =
          universeSettings.uploadGet + resp.filename + "/lg"; // #Universe
    ...
    function upload(images, $image_placeholder) {
      ...
      if (typeof images != "undefined" && images.length > 0) {
        ...
        if (
          editor.events.trigger("image.beforeUpload", [
            images,
            $image_placeholder
          ]) === false
        ) {
        ...
        if (
          (editor.opts.imageUploadURL === null ||
            editor.opts.imageUploadURL == DEFAULT_IMAGE_UPLOAD_URL) &&
          !editor.opts.imageUploadToS3
        ) {
        ...
        if (!image.name) {
          image.name =
            new Date().getTime() +
            "." +
            (image.type || "image/jpeg").replace(/image\//g, "");
        ...
        if (
          editor.opts.imageAllowedTypes.indexOf(
            image.type.replace(/image\//g, "")
          ) < 0
        ) {
        ...
        if (form_data) {
          ...
          if (editor.opts.imageUploadToS3 !== false) {
            form_data.append(
              "key",
              editor.opts.imageUploadToS3.keyStart +
                new Date().getTime() +
                "-" +
                (image.name || "untitled")
            );
          ...
          if (editor.opts.imageUploadToS3) {
            if (editor.opts.imageUploadToS3.uploadURL) {
              ...
              url =
                "https://" +
                editor.opts.imageUploadToS3.region +
                ".amazonaws.com/" +
                editor.opts.imageUploadToS3.bucket;
            }
          }
          ...
          _startUpload(
            ...
            form_data,
            image,
            $image_placeholder || $current_image
          );
        }
      }
    }
    ...
    function _initEvents() {
      ...
      if (editor.opts.imageAddNewLine) {
        editor.events.on("image.inserted", function($img) {
          ...
          if (lastNode.nextSibling && lastNode.nextSibling.tagName === "BR")
            lastNode = lastNode.nextSibling; // Look upper nodes.
    ...
    function _initInsertPopup(delayed) {
      ...
      if (
        !editor.opts.imageUpload &&
        editor.opts.imageInsertButtons.indexOf("imageUpload") !== -1
      ) {
        editor.opts.imageInsertButtons.splice(
          editor.opts.imageInsertButtons.indexOf("imageUpload"),
          1
        );
      }
      ...
      if (buttonList !== "") {
        image_buttons =
          '<div class="fr-buttons fr-tabs">' + buttonList + "</div>";
      }
      ...
      if (uploadIndex >= 0) {
        ...
        upload_layer =
          ...
          editor.id +
          '"><strong>' +
          editor.language.translate("Drop image") +
          "</strong><br>(" +
    ...
    function _initAltPopup(delayed) {
      ...
      image_buttons =
        ...
        editor.button.buildList(editor.opts.imageAltButtons) +
        "</div>"; // Image by url layer.
      ...
      alt_layer =
        ...
        editor.language.translate("Alternative Text") +
      ...
      if ($current_image) {
        if (hasCaption()) {
          ...
          $popup
            ...
            .val($el.get(0).style.width)
            .trigger("change");
          $popup
            .find('input[name="height"]')
            .val($el.get(0).style.height)
            .trigger("change");
        } else {
          $popup
            .find('input[name="width"]')
            .val($current_image.get(0).style.width)
            .trigger("change");
          $popup
            .find('input[name="height"]')
            .val($current_image.get(0).style.height)
            .trigger("change");
        }
      }
    }
    ...
    function _initSizePopup(delayed) {
      ...
      image_buttons =
        '<div class="fr-buttons fr-tabs">' +
        editor.button.buildList(editor.opts.imageSizeButtons) +
        "</div>"; // Size layer.
      ...
      if ($current_image) {
        ...
        if (hasCaption()) {
          ...
          if (height.match(regex))
            $current_image.parents(".fr-img-caption").css("height", height);
    ...
    function showLayer(name) {
      ...
      if (!$current_image && editor.opts.toolbarInline) {
        ...
        top =
          $popup.offset().top - editor.helpers.getPX($popup.css("margin-top")); // If the popup is above apply height correction.
      ...
      editor.popups.show(
        ...
        left,
        top,
        $current_image ? $current_image.outerHeight() : 0
      );
    ...
    function refreshUploadButton($btn) {
      ...
      if (
        $popup &&
        $popup.find(".fr-image-upload-layer").hasClass("fr-active")
      ) {
    ...
    function refreshByURLButton($btn) {
      ...
      if (
        $popup &&
        $popup.find(".fr-image-by-url-layer").hasClass("fr-active")
      ) {
        $btn.addClass("fr-active").attr("aria-pressed", true);
      }
    }
    ...
    function _initImageResizer() {
      ...
      if (!editor.shared.$image_resizer) {
        ...
        editor.shared.$image_resizer = $(document.createElement("div")).attr(
          "class",
          "fr-image-resizer"
        );
        ...
        if (editor.opts.imageResize) {
          ...
          editor.shared.$img_overlay = $(document.createElement("div")).attr(
            "class",
            "fr-image-overlay"
          );
        ...
        editor.events.on(
          ...
          function() {
            $("body")
              .first()
              .append($image_resizer.removeClass("fr-active"));
          },
          true
        );
      } // Shared destroy.
        ...
        editor.events.on(
          ...
          function(e) {
            if ($current_image) {
              ...
              if (
                (keycode == FE.KEYCODE.EQUALS ||
                  (editor.browser.mozilla &&
                    keycode == FE.KEYCODE.FF_EQUALS)) &&
                ctrlKey &&
                !e.altKey
              ) {
                ...
                (keycode == FE.KEYCODE.HYPHEN ||
                  (editor.browser.mozilla &&
                    keycode == FE.KEYCODE.FF_HYPHEN)) &&
                ctrlKey &&
                !e.altKey
              ) {
      ...
      if ($img) {
        if (editor.events.trigger("image.beforeRemove", [$img]) !== false) {
          ...
          if ($img.get(0) == editor.el) {
            ...
            if (
              $img.get(0).parentNode &&
              $img.get(0).parentNode.tagName == "A"
            ) {
              editor.selection.setBefore($img.get(0).parentNode) ||
                editor.selection.setAfter($img.get(0).parentNode) ||
                $img.parent().after(FE.MARKERS);
              $($img.get(0).parentNode).remove();
            } else {
              editor.selection.setBefore($img.get(0)) ||
                editor.selection.setAfter($img.get(0)) ||
                $img.after(FE.MARKERS);
              $img.remove();
            }
    ...
    function _editorKeydownHandler(e) {
      ...
      if (
        $current_image &&
        (key_code == FE.KEYCODE.BACKSPACE || key_code == FE.KEYCODE.DELETE)
      ) {
        e.preventDefault();
        e.stopPropagation();
        remove();
        ...
        $current_image &&
        (key_code == FE.KEYCODE.ARROW_LEFT ||
          key_code == FE.KEYCODE.ARROW_RIGHT)
      ) {
    ...
    function _cleanOnGet(el) {
      ...
      if (el && el.tagName == "IMG") {
        ...
        if (
          editor.node.hasClass(el, "fr-uploading") ||
          editor.node.hasClass(el, "fr-error")
        ) {
        ...
        if (
          ...
          editor.node.hasClass(el.parentNode.parentNode, "fr-img-caption")
        ) {
    ...
    function _init() {
      ...
      editor.events.$on(
        editor.$el,
        editor.helpers.isMobile() && !editor.helpers.isWindowsPhone()
          ? "touchend"
          : "click",
        editor.el.tagName == "IMG"
          ? null
      ...
      editor.events.on(
        ...
        function(e) {
          ...
          if (
            $current_image &&
            editor.popups.isVisible("image.edit") &&
            !editor.popups.get("image.edit").find(":focus").length
          ) {
            var $el = getEl();
            ...
            if (hasCaption()) {
              ...
              editor.paste.saveCopiedText(
                $current_image.get(0).outerHTML,
      ...
      editor.events.$on(editor.$win, "keydown", function(e) {
        ...
        if (
          $current_image &&
          $current_image.hasClass("fr-uploading") &&
          key_code == FE.KEYCODE.ESC
        ) {
      ...
      if (editor.opts.imageOutputSize) {
        ...
        editor.events.on("html.beforeGet", function() {
          ...
          for (var i = 0; i < imgs.length; i++) {
            ...
            if (width)
              imgs[i].setAttribute("width", ("" + width).replace(/px/, ""));
            if (height)
              imgs[i].setAttribute("height", ("" + height).replace(/px/, ""));
          }
        });
      }
        ...
        editor.$el.find("img[data-fr-image-pasted]").each(function(index, img) {
          ...
          if (img.src.indexOf("data:") === 0) {
            ...
            _img.src =
              (img.src.indexOf("blob:") === 0
                ? ""
                : editor.opts.imageCORSProxy + "/") + img.src;
          } // Images without http (Safari ones.).
          else if (
            img.src.indexOf("http") !== 0 ||
            img.src.indexOf("https://mail.google.com/mail") === 0
          ) {
        ...
        if (
          !(
            (e.clipboardData.types &&
              e.clipboardData.types.indexOf("text/rtf") != -1) ||
        ...
        if (_display == "inline") {
          ...
          if (_align == "center") {
            $img.css({
              ...
              marginTop: "",
              maxWidth:
                "calc(100% - " + 2 * editor.opts.imageDefaultMargin + "px)",
              textAlign: "center"
            });
          } else if (_align == "left") {
            $img.css({
    ...
    function refreshAlign($btn) {
      if ($current_image) {
        $btn
          .find("> *")
          .first()
          .replaceWith(editor.icon.create("image-align-" + getAlign()));
      }
    }
    /**
    ...
    function refreshAlignOnShow($btn, $dropdown) {
      if ($current_image) {
        $dropdown
          .find('.fr-command[data-param1="' + getAlign() + '"]')
          .addClass("fr-active")
    ...
    function refreshDisplayOnShow($btn, $dropdown) {
      if ($current_image) {
        $dropdown
          .find('.fr-command[data-param1="' + getDisplay() + '"]')
          .addClass("fr-active")
          .attr("aria-selected", true);
      }
    }
    ...
    function getEl() {
      return hasCaption()
        ? $current_image.parents(".fr-img-caption").first()
        : $current_image;
    }
    /**
    ...
    function applyStyle(val, imageStyles, multipleStyles) {
      if (typeof imageStyles == "undefined")
        imageStyles = editor.opts.imageStyles;
    ...
    function toggleCaption() {
      ...
      if ($current_image && !hasCaption()) {
        ...
        $el.wrap(
          "<span " +
            (!editor.browser.mozilla ? 'contenteditable="false"' : "") +
            'class="fr-img-caption ' +
            $current_image.attr("class") +
            '" draggable="false"></span>'
        );
        $el.wrap('<span class="fr-img-wrap"></span>');
        $el.after(
          '<span class="fr-inner"' +
            (!editor.browser.mozilla ? ' contenteditable="true"' : "") +
            ">" +
            FE.START_MARKER +
            "Image caption" +
            FE.END_MARKER +
      ...
      for (var val in options) {
        if (options.hasOwnProperty(val)) {
          c +=
            ...
            '">' +
            this.icon.create("image-align-" + val) +
